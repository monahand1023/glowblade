...
2. `pip install -e ".[dev]"`
3. `lightsaber-fx setup` -- clones SAM2, installs it, and downloads the
...
- **Glow shape** (`CORE_BLUR`, `INNER_GLOW_BLUR`, `SPILL_BLUR`) and **whoosh
  sensitivity** are still internal constants in
  `src/lightsaber_fx/pipeline/glow.py` and
  `src/lightsaber_fx/pipeline/audio.py` -- edit them directly if needed.
